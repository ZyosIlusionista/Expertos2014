	...
	jQuery.extend(peticion, {
		...
		peticionAjax: function(formulario) {
			jQuery.ajax({
				url : "{{ RUTA_APP|e }}/Permisos/Nuevo/accesoProcesar",
				async : false,
		...
		success : function(resultado) {
			if(resultado.status == true || resultado.codigo >= 1) {
				swal({
					...
					text: "Se ha creado con exito el Acceso con el ID: " + resultado.codigo, 
					type: "success", 
					...
					closeOnCancel: false
						...
						location.href="{{ RUTA_APP|e }}/Permisos/Index/accesos";
					});
			}
			else {
