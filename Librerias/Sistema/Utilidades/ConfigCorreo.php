<?php
	...
	class ConfigCorreo {
		 ...
		 * @return bool
		 */
		public static function confgExistencia($confg = false) {
			if(is_array(self::$config) == true):
				return self::appExistenciaValidar($confg);
			else:
		...
		private static function seleccion($parametros = false) {
			...
			foreach ($parametros AS $param):
				if(array_key_exists($param, $base) == true):
					...
					throw new Excepcion(sprintf('El apartado %s no existe en la configuración de correo', $param), 0);
					break;
				endif;
			endforeach;
